...
#
# Shamelessly stolen from rodjek/logrotate on puppet forge. Retooled to our
# format but mostly left intact.
#
# == Parameters:
#
# == Required:
...
# == Optional:
#
# installed::       The desired state of the logrotate rule as a Bool.  Valid
#                   values are 'true' and 'false' (default: 'true').
#
  ...
  case $installed {
    true: {
      file { "/etc/logrotate.d/${name}":
        ...
        group   => 'root',
        mode    => '0644',
        content => template('logrotate/Ubuntu/rule.erb'),
        tag     => "${name}-logrotate-rule"
      }
    }
    false: {
      file { "/etc/logrotate.d/${name}": ensure => 'absent' }
    }
    default: {}
  }
}
