...
# And
Given /^a fact? "([^\"]*)" of "([^\"]*)"$/ do |fact, value|
  @facts[fact] = value
end

Given /^without fact? "([^\"]*)"$/ do |fact|
  @facts.delete(fact) if @facts[fact]
end
...
Then /^the [a-z]* should run before "([^\"]*)"$/ do |res|
  req = @resource["before"]
  if req.is_a?(Array)
    found = false
    req.each do |r|
    ...
    fail("#{@resource} should run before #{res}") unless found
  else
    fail("#{@resource} should run before #{res}") unless req.to_s == res
  end
  steps %Q{
    Then the catalog should contain "#{res}"
  }
...
Then /^the state should be "([^\"]*)"$/ do |state|
  fail("resource #{@resource} had state #{@resource["ensure"]}") unless @resource["ensure"] == state
end
