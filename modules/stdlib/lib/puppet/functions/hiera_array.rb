module Puppet::Parser::Functions
    newfunction(:hiera_array, :type => :rvalue) do |*args|
        ...
        # Stupid hack to get the data dir to be set relative to the puppet.conf
        # if it isn't absolute. Useful for running cucumber-puppet tests.
        unless config[:json].nil?
          unless config[:json][:datadir] =~ /^\/.*$/
            config[:json][:datadir] = File.join([File.dirname(Puppet.settings[:config]), config[:json][:datadir]])
          end
        end
        ...
        Hiera.warn("Hiera_array says the answer is #{answer.inspect}")
